...
class Gridworld():
    ...
    def performAction(self, currentPos, actionNr): # 0 = L,  1 = U, 2 = R, 3 = D
        ...
        if actionNr == 2: # R
            if currentPos[0] + 1 < self.__envDim[0]:
                newPos = (currentPos[0] + 1, currentPos[1])

        if actionNr == 3: # D
            if currentPos[1] + 1 < self.__envDim[1]:
                newPos = (currentPos[0], currentPos[1] + 1)
    ...
    def __getReward(self, tilePosition):
        ...
        if tilePosition == self.__endTiles:
            return self.__goalR

        for i in range(len(self.__trapTiles)):
            if tilePosition == self.__trapTiles[i]:
                return self.__trapR
    ...
    def __drawGrid(self):
        ...
        for x in range(self.__envDim[0]):
            pygame.draw.line(self.__window, color, (x * self.__sqSize, 0), (x * self.__sqSize, self.__envDim[1] * self.__sqSize),
                             width)
        for y in range(self.__envDim[1]):
            pygame.draw.line(self.__window, color, (0, y * self.__sqSize), (self.__envDim[0] * self.__sqSize, y * self.__sqSize),
                             width)
